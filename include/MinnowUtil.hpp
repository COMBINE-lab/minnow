...
namespace util{
 ...
 // Follows the different start position distribution
  ...
  // Stream the command line options
  // to a file 
  inline void writeCmdInfo(SimulateOptions& simOpts){
    std::string outDir = simOpts.outDir ;
    std::string cmdInfoPath = outDir + "/cmd_info.txt" ;



  }

  // This models are generated by https://github.com/alyssafrazee/polyester/blob/master/build_error_models.py
