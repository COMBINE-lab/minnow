...
template<class G>
void GenerateGfaOutput(const std::string & inputFileName, const std::vector<std::string> & genomes, size_t k, bool prefix, const std::string & outFileName, G g)
{	
	std::vector<uint64_t> chrSegmentLength;
	std::vector<std::string> chrSegmentId;
	std::map<std::string, std::string> chrFileName;

	std::ofstream gfaStream(outFileName) ;
	//std::cout << "H\tVN:Z:1.0" << std::endl;
	g.Header(gfaStream);

	ReadInputSequences(genomes, chrSegmentId, chrSegmentLength, chrFileName, !prefix);
	g.ListInputSequences(chrSegmentId, chrFileName, gfaStream);
		...
		while (reader.NextJunctionPosition(end))
		{
			if (begin.GetChr() == end.GetChr())
			{
				...
				if (!seen[Abs(segmentId)])
				{
					...
					g.Segment(segmentId, segmentSize, ss.str(), gfaStream);
...
#endif
				g.Occurrence(segmentId, segmentSize, chrSegmentId[seqId], chrSegmentLength[seqId], begin.GetPos(), end.GetPos(), k, gfaStream);
				...
				if (prevSegmentId != NO_SEGMENT)
				{
					...
					g.Edge(prevSegmentId, prevSegmentSize, segmentId, segmentSize, k, gfaStream);
				}				
...
int dumpGraphMain(std::vector<std::string>& args)//main(int argc, char * argv[])
{
	...
	try
	{
		...
		//TCLAP::SwitchArg outfile("", "outfile", "output stream to hold the gfa file", cmd, false);
		...
		else if (outputFileFormat.getValue() == format[3])
		{
			...
			GenerateGfaOutput(inputFileName.getValue(), seqFileName.getValue(), kvalue.getValue(), prefix.getValue(), outFileName.getValue(), Gfa1Generator());
		}
		else if (outputFileFormat.getValue() == format[4])
		{
			...
			GenerateGfaOutput(inputFileName.getValue(), seqFileName.getValue(), kvalue.getValue(), prefix.getValue(), outFileName.getValue(), Gfa2Generator());
		}
